...
void mapaLaboratorio() {
    // 7x20, "P"=puerta salida, "O"=prof, "1-3"=pokebolas
    const int H=7, W=20;
    char mapa[H][W+1] = {
        "####################",
        "#   O     1 2 3   P#",
    ...
    bool starterOK = starterYaEscogido;

    while (true) {
        system("cls");
        cout << "[Laboratorio] Usa WASD para moverte y elige tu cuscamon. Sal por la puerta (P).\n";
        for (int y=0; y<H; y++) {
            for (int x=0; x<W; x++) {
        ...
        if (t == 'q') { salirAlMenu = true; return; }
        // Interactuar con Profesor
        if (mapa[py][px]=='O') {
            setColor(13);
        ...
        // Elegir pokebola
        if (mapa[py][px]>='1' && mapa[py][px]<='3' && !starterOK) {
            int pid=mapa[py][px]-'1';
            setColor(10); cout << "\nPreview: " << pokedex[pid].nombre << "\n"; setColor(7);
            cout << pokedex[pid].ascii << endl;
            cout << "Tipo: " << pokedex[pid].tipo << "\n";
            cout << pokedex[pid].desc << endl;
            cout << "¿Quieres este? (Y/N): ";
            char c; cin >> c;
            if (c=='Y'||c=='y') {
                equipo[numEquipo++] = pid;
                pokedex[pid].atrapado=true; pokesVistos[pid]=1;
                starterOK = true; starterYaEscogido = true;
                setColor(10); cout << "¡Ahora " << pokedex[pid].nombre << " es tu compañero!\n"; setColor(7);
                Sleep(1200);
            } else { cout << "Sigue buscando el que mas te guste.\n"; Sleep(800); }
        }
        else if (mapa[py][px]>='1' && mapa[py][px]<='3' && starterOK) {
            setColor(12);
            cout << "\nYa elegiste tu Cuscamon, no puedes tomar otro.\n";
            setColor(7);
            Sleep(1000);
        }
        // Salida
        if (mapa[py][px]=='P' && starterOK) {
            animacionCarga(); break;
        }
    }
}
